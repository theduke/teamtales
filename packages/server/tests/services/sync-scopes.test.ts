...
describe("addSyncScopeService", mysqlTestOptions, () => {
  it("rejects sync scope types that do not match the provider and persists compatible scopes", async () => {
    ...
    const suffix = uniqueId("scope");
    const organizationId = `org_${suffix}`,
      userId = `user_${suffix}`,
      githubId = `github_${suffix}`,
      linearId = `linear_${suffix}`;
    try {
      await createOrganizationService(opened.db, {
        id: organizationId,
        name: "Service Org",
        slug: organizationId,
        ownerId: userId,
      ...
      await opened.db.insert(integrations).values([
        ...
        },
        {
          ...
          authType: "personal_access_token",
          status: "active",
          displayName: linearId,
        },
      ]);
      await assert.rejects(
        addSyncScopeService(opened.db, {
          organizationId,
          userId,
