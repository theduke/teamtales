...
describe("MySQL configuration", () => {
  it("accepts Wasmer capability variables and DB_USERNAME precedence", () => {
    const options = mysqlConnectionOptions({ DB_HOST: "mysql.internal", DB_PORT: "3307", DB_USERNAME: "wasmer-user", DB_USER: "fallback", DB_PASSWORD: "secret", DB_NAME: "teamtales" });
...
it("runs migrations and meaningful Drizzle operations against MySQL", { skip: !testDatabaseUrl }, async () => {
  const opened = await openDatabase({ env: { DATABASE_URL: testDatabaseUrl }, runMigrations: true });
  const suffix = randomUUID();
  const organizationId = `org_${suffix}`;
  const userId = `user_${suffix}`;
  try {
    const created = await createOrganizationService(opened.db, { id: organizationId, name: `Test ${suffix}`, slug: `test-${suffix}`, ownerId: userId, ownerEmail: `${suffix}@example.test`, ownerName: "Test Owner", membershipId: `membership_${suffix}` });
    assert.equal(created.ownerUserId, userId);
    await setPassword(opened.db, userId, "correct horse battery staple");
    assert.equal((await authenticatePassword(opened.db, `${suffix}@example.test`, "correct horse battery staple"))?.userId, userId);
    const integration = await addPersonalAccessTokenIntegrationService(opened.db, { id: `integration_${suffix}`, credentialId: `credential_${suffix}`, organizationId, userId, provider: "github", displayName: "Test GitHub", token: "github_pat_integration_test", encryptionKey: "0123456789abcdef0123456789abcdef" });
    const scope = await addSyncScopeService(opened.db, { id: `scope_${suffix}`, organizationId, userId, integrationId: integration.id, provider: "github", scopeType: "github.repository", externalName: "owner/repository", config: { owner: "owner", repo: "repository" } });
    const [storedScope] = await opened.db.select().from(syncScopes).where(eq(syncScopes.id, scope.id));
    assert.equal(storedScope?.integrationId, integration.id);
    assert.deepEqual(JSON.parse(storedScope?.configJson ?? "{}"), { owner: "owner", repo: "repository" });
  } finally {
