...
describe("MySQL authentication", mysqlTestOptions, () => {
  ...
  it("sets a scrypt password and authenticates email case-insensitively", async () => {
    ...
    try {
      ...
      const principal = await authenticatePassword(
        local.db,
      ...
      assert.equal(
        await authenticatePassword(local.db, local.email, "incorrect password"),
        undefined,
      );
      ...
      await local.db.delete(users).where(eq(users.id, local.userId));
      await local.close();
    }
  });
  ...
  it("rejects weak passwords, unknown users, and users without a password", async () => {
    ...
    try {
      ...
      await assert.rejects(
        setPassword(local.db, uniqueId("missing"), "long enough password"),
        /not found/,
      );
      assert.equal(await authenticatePassword(local.db, local.email, "anything at all"), undefined);
      assert.equal(
        await authenticatePassword(local.db, "missing@example.test", "anything at all"),
        undefined,
      );
    } finally {
      await local.db.delete(users).where(eq(users.id, local.userId));
  ...
  it("stores only a session hash, resolves active sessions, and revokes idempotently", async () => {
    ...
    try {
      const created = await createSession(local.db, local.userId, {
        now,
      ...
      const [row] = await local.db
        .select()
        .from(authSessions)
        .where(eq(authSessions.id, created.session.id));
      assert.equal(JSON.stringify(row).includes(created.token), false);
      assert.match(row!.tokenHash, /^sha256:[a-f0-9]{64}$/);
      ...
      await assert.rejects(
        createSession(local.db, local.userId, { now, expiresAt: now }),
        /future/,
      );
    } finally {
      await local.db.delete(users).where(eq(users.id, local.userId));
  ...
  it("rejects expired sessions and invalid expiry dates", async () => {
    ...
    try {
      ...
      assert.equal(
        await resolveSession(local.db, created.token, {
  ...
  it("returns an API token once, tracks usage, and revokes by token or id", async () => {
    ...
    const now = new Date("2026-07-09T10:00:00.000Z");
    try {
      const created = await createApiToken(local.db, local.userId, {
        name: "automation",
        now,
      ...
      const [row] = await local.db
        .select()
        .from(apiTokens)
        .where(eq(apiTokens.id, created.apiToken.id));
      assert.equal(JSON.stringify(row).includes(created.token), false);
      assert.equal(row?.tokenPrefix, created.apiToken.prefix);
      const usedAt = new Date("2026-07-09T10:05:00.000Z");
      assert.equal(
        (await resolveApiToken(local.db, created.token, { now: usedAt }))?.userId,
        local.userId,
      );
      assert.equal(
        (await local.db.select().from(apiTokens).where(eq(apiTokens.id, created.apiToken.id)))[0]
          ?.lastUsedAt,
        usedAt.toISOString(),
      );
  ...
  it("rejects empty token names, missing users, and expired tokens", async () => {
    ...
    try {
      await assert.rejects(createApiToken(local.db, local.userId, { name: " ", now }), /name/);
      await assert.rejects(
        createApiToken(local.db, uniqueId("missing"), { name: "test", now }),
        /not found/,
      );
      const created = await createApiToken(local.db, local.userId, {
        name: "short lived",
      ...
      assert.equal(
        await resolveApiToken(local.db, created.token, {
          now: new Date("2026-07-09T10:00:01.000Z"),
        }),
        undefined,
      );
    } finally {
