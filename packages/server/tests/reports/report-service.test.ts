...
describe("report service", mysqlTestOptions, () => {
  it("persists generated analysis highlights from the report context", async () => {
    ...
    const suffix = uniqueId("report"),
      organizationId = `org_${suffix}`,
      workItemId = `work_${suffix}`;
    const context: ReportContext = {
      organization: { id: organizationId, name: "Report Org" },
      scope: { type: "github_repository", id: `repo_${suffix}`, name: "acme/report" },
    ...
    try {
      await opened.db
        ...
        .values({ id: organizationId, name: "Report Org", slug: organizationId });
      ...
      const generated = await generateWeeklyReportService(opened.db, {
        context,
        persist: true,
        analysisRunIdSeed: suffix,
      });
      const [storedContext] = await opened.db
        .select()
        .from(analysisReportContexts)
        .where(eq(analysisReportContexts.id, generated.analysisReportContextId));
      const highlights = await listAnalysisHighlights(opened.db, storedContext!.analysisRunId);
      assert.equal(highlights.length, 1);
      assert.equal(highlights[0]?.workItemId, workItemId);
