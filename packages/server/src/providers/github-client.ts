...
export class GitHubRestClient {
  ...
  async getObject(path: string, query: Record<string, string> = {}): Promise<GitHubObject> {
    ...
    if (!isJsonObject(value)) throw new Error(`GitHub API returned a non-object response for ${path}`);
    return value;
  }
  ...
  async *paginateObjects(path: string, query: Record<string, string> = {}): AsyncGenerator<GitHubObject> {
    ...
    while (url) {
      ...
      if (!Array.isArray(value)) throw new Error(`GitHub API returned a non-array response for ${path}`);
      for (const item of value) {
        if (!isJsonObject(item)) throw new Error(`GitHub API returned a non-object item for ${path}`);
        yield item;
      }
...
function nextLink(linkHeader: string | null): string | undefined {
  ...
  return linkHeader.split(",").map(part => part.trim()).map(part => /<([^>]+)>;\s*rel="([^"]+)"/.exec(part)).find(match => match?.[2] === "next")?.[1];
}
...
export async function githubApiError(response: Response): Promise<Error> {
  ...
  try { const body: unknown = await response.json(); if (isJsonObject(body) && typeof body.message === "string") message = body.message; } catch { /* retain status */ }
  const prefix = response.status === 403 && remaining === "0" ? `GitHub API rate limit exceeded${reset ? ` until ${new Date(Number(reset) * 1000).toISOString()}` : ""}` : `GitHub API request failed with ${response.status} ${response.statusText}`;
  return new Error(message ? `${prefix}: ${message}` : prefix);
}

function isJsonObject(value: unknown): value is GitHubObject { return typeof value === "object" && value !== null && !Array.isArray(value); }
function isJsonValue(value: unknown): value is JsonValue { return value === null || typeof value === "string" || typeof value === "number" || typeof value === "boolean" || (Array.isArray(value) && value.every(isJsonValue)) || isJsonObject(value) && Object.values(value).every(isJsonValue); }
