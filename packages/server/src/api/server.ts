...
async function handleRequest(
  ...
  try {
    ...
    if (!url.pathname.startsWith("/api/")) {
      serveUi(
        response,
        options.uiDirectory ?? join(process.cwd(), "packages/ui/dist"),
        url.pathname,
      );
      return;
    }
...
function serveUi(response: ServerResponse, directory: string, pathname: string): void {
  ...
  if (!existsSync(filename)) {
    ...
    response.end("TeamTales UI has not been built.");
    return;
  }
  ...
  response.writeHead(200, {
    ...
    "cache-control":
      extname(filename) === ".html" ? "no-cache" : "public, max-age=31536000, immutable",
  });
