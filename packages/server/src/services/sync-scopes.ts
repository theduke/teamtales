...
type BaseInput = Pick<
  AddSyncScopeServiceInput,
  "organizationId" | "userId" | "integrationId" | "provider"
>;

export async function addSyncScopeService(
  database: AppDatabase,
  input: AddSyncScopeServiceInput,
): Promise<SyncScopeDto> {
  await authorize(database, input);
  if (!input.scopeType.startsWith(`${input.provider}.`))
    throw new Error(
      `Sync scope type ${input.scopeType} is not supported for provider ${input.provider}.`,
    );
  const now = new Date().toISOString();
  const id = input.id ?? scopeId(input, input.scopeType, input.externalId ?? input.externalName);
...
export async function setGitHubScopeSelectionService(
  database: AppDatabase,
  input: BaseInput & {
    selection: {
      organizations: Array<
  ...
  await authorize(database, input);
  if (input.provider !== "github")
    throw new Error("GitHub selection requires a GitHub integration.");
  const organizations = new Map(input.discovery.organizations.map((value) => [value.id, value]));
  const repositories = new Map(input.discovery.repositories.map((value) => [value.id, value]));
  const orgIds = new Set<string>();
  const repoIds = new Set<string>();
  for (const item of input.selection.organizations) {
    if (orgIds.has(item.organizationId)) throw new Error("Duplicate organization selection.");
    orgIds.add(item.organizationId);
    const org = organizations.get(item.organizationId);
    if (!org) throw new Error("Unknown or inaccessible GitHub organization.");
    if (item.mode === "selected")
      for (const repositoryId of item.repositoryIds) {
        if (repoIds.has(repositoryId)) throw new Error("Duplicate repository selection.");
        repoIds.add(repositoryId);
        const repository = repositories.get(repositoryId);
        if (!repository || repository.organizationId !== org.id)
          throw new Error("Repository does not belong to the selected organization.");
      }
  }
  for (const repositoryId of input.selection.repositoryIds) {
    if (repoIds.has(repositoryId)) throw new Error("Repository selected more than once.");
    repoIds.add(repositoryId);
    const repository = repositories.get(repositoryId);
    if (!repository) throw new Error("Unknown or inaccessible GitHub repository.");
    if (
      [...orgIds].some(
        (id) =>
          organizations.get(id)?.id === repository.organizationId &&
          input.selection.organizations.find((item) => item.organizationId === id)?.mode === "all",
      )
    )
      throw new Error("A repository is already covered by an all-repositories organization.");
  }
  await database.transaction(async (tx) => {
    ...
    const existing = await tx
      ...
      .from(syncScopes)
      .where(
        and(eq(syncScopes.integrationId, input.integrationId), eq(syncScopes.provider, "github")),
      );
    const organizationResources = new Map<string, string>();
    for (const organization of organizations.values())
      organizationResources.set(
        organization.id,
        await upsertProviderResource(tx, input, {
          ...
          metadata: {
            ...
            repositoryCount: organization.repositoryCount ?? null,
          },
          now,
        }),
      );
    ...
    for (const repository of repositories.values())
      repositoryResources.set(
        repository.id,
        await upsertProviderResource(tx, input, {
          resourceType: "github.repository",
          externalId: repository.id,
          externalParentId: repository.organizationId,
          parentResourceId: repository.organizationId
            ? organizationResources.get(repository.organizationId)
            : undefined,
    ...
    for (const item of input.selection.organizations) {
      const org = organizations.get(item.organizationId)!;
      const id = scopeId(input, "github.organization", org.id);
      desired.add(id);
      await upsert(tx, {
        id,
        organizationId: input.organizationId,
      ...
      if (item.mode === "selected")
        for (const repositoryId of item.repositoryIds) {
          ...
          const childId = scopeId(input, "github.repository", repo.id);
          desired.add(childId);
          await upsert(tx, {
            id: childId,
            organizationId: input.organizationId,
    ...
    for (const repositoryId of input.selection.repositoryIds) {
      const repo = repositories.get(repositoryId)!;
      const id = scopeId(input, "github.repository", repo.id);
      desired.add(id);
      await upsert(tx, {
        id,
        organizationId: input.organizationId,
    ...
    for (const row of existing)
      if (!desired.has(row.id))
        await tx
          ...
          .where(eq(syncScopes.id, row.id));
  });
  return scopesForIntegration(database, input.organizationId, input.integrationId);
}

export async function setLinearScopeSelectionService(
  database: AppDatabase,
  input: BaseInput & {
  ...
  if (input.provider !== "linear")
    throw new Error("Linear selection requires a Linear integration.");
  const teams = new Map(input.discovery.teams.map((team) => [team.id, team]));
  const teamIds = input.selection.mode === "selected" ? input.selection.teamIds : [];
  if (new Set(teamIds).size !== teamIds.length || teamIds.some((id) => !teams.has(id)))
    throw new Error("Unknown, inaccessible, or duplicate Linear team.");
  await database.transaction(async (tx) => {
    const now = new Date().toISOString();
    const workspaceResourceId = await upsertProviderResource(tx, input, {
      resourceType: "linear.workspace",
      externalId: input.discovery.workspace.id,
      displayName: input.discovery.workspace.name,
      now,
    });
    const teamResources = new Map<string, string>();
    for (const team of teams.values())
      teamResources.set(
        ...
        await upsertProviderResource(tx, input, {
          ...
          externalId: team.id,
          externalParentId: input.discovery.workspace.id,
    ...
    const workspaceId = scopeId(input, "linear.workspace", input.discovery.workspace.id);
    const desired = new Set([workspaceId]);
    await upsert(tx, {
      id: workspaceId,
      organizationId: input.organizationId,
      integrationId: input.integrationId,
    ...
    for (const teamId of teamIds) {
      ...
      const id = scopeId(input, "linear.team", team.id);
      desired.add(id);
      await upsert(tx, {
        id,
        organizationId: input.organizationId,
    ...
    const existing = await tx
      ...
      .from(syncScopes)
      .where(
        and(eq(syncScopes.integrationId, input.integrationId), eq(syncScopes.provider, "linear")),
      );
    for (const row of existing)
      if (!desired.has(row.id))
        await tx
...
async function authorize(database: AppDatabase, input: BaseInput): Promise<void> {
...
function scopeId(
  input: Pick<BaseInput, "organizationId" | "integrationId">,
  scopeType: SyncScopeDto["scopeType"],
  externalId: string,
): string {
  return stableId("scope", input.organizationId, input.integrationId, scopeType, externalId);
}
async function upsert(tx: any, value: any): Promise<void> {
  const existing = await tx
    .select({ id: syncScopes.id })
    .from(syncScopes)
    .where(eq(syncScopes.id, value.id))
    .limit(1);
  if (existing[0])
    await tx
      .update(syncScopes)
      ...
      .where(eq(syncScopes.id, value.id));
  else await tx.insert(syncScopes).values(value);
}
async function upsertProviderResource(
  ...
  const existing = await tx
    ...
    .where(
      and(
        eq(providerResources.integrationId, input.integrationId),
        eq(providerResources.resourceType, value.resourceType),
        eq(providerResources.externalId, value.externalId),
      ),
    )
    .limit(1);
  const resource = {
    ...
    externalId: value.externalId,
    externalParentId: value.externalParentId ?? null,
    parentResourceId: value.parentResourceId ?? null,
    displayName: value.displayName,
  ...
  else await tx.insert(providerResources).values({ ...resource, createdAt: value.now });
  return resource.id;
}
async function scopesForIntegration(
  database: AppDatabase,
  organizationId: string,
  integrationId: string,
): Promise<SyncScopeDto[]> {
  const rows = await database
    ...
    .where(
      and(
        eq(syncScopes.organizationId, organizationId),
        eq(syncScopes.integrationId, integrationId),
      ),
    );
  return rows.map((row) => ({
    id: row.id,
...
function toDto(value: any): SyncScopeDto {
  return {
    id: value.id,
    ...
    ...(value.parentScopeId ? { parentScopeId: value.parentScopeId } : {}),
